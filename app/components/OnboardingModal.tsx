...
export function OnboardingModal({ onSave }: Props) {
  ...
  async function handleAvatarUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/avatar/create", { method: "POST", body: form });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 w-full max-w-lg my-4">
        ...
        <div className="space-y-5">
          ...
          {/* Avatar — optional, enables Speaker mode */}
          <div className="border border-gray-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-300">AI Avatar <span className="text-xs text-gray-500 font-normal ml-1">optional — enables Speaker mode</span></p>
                <p className="text-xs text-gray-500 mt-0.5">Record 5-15s of yourself speaking. HeyGen clones your likeness.</p>
              </div>
              {avatarState === "done" && (
                <span className="text-xs text-green-400 font-medium shrink-0 ml-3">✓ Ready</span>
              )}
            </div>

            {avatarState === "done" ? (
              <button
                onClick={() => { setAvatarId(undefined); setAvatarState("idle"); }}
                className="text-xs text-gray-500 hover:text-gray-300 transition-colors"
              >
                Re-record avatar
              </button>
            ) : (
              <label className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border text-sm cursor-pointer transition-colors ${
                avatarState === "uploading"
                  ? "border-gray-700 text-gray-500 cursor-not-allowed"
                  : "border-gray-700 text-gray-300 hover:border-pink-500 hover:text-white"
              }`}>
                {avatarState === "uploading" ? (
                  <>
                  ...
                  <>
                    <span>🎥</span> Upload reference video
                  </>
                )}
                <input
                  type="file"
        ...
        <button
          disabled={!canSave}
          onClick={() => onSave({ niche: effectiveNiche, styleDescription, voiceId, ttsProvider, avatarId })}
